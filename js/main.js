...
if ((mobileStyle == 'modern' || mobileStyle == 'full-screen-menu') && !$('.navbar-' + mobileStyle + '-inner').length) {
    ...
    /* ===================================
     ...
     if ($('.magic-cursor').length > 0) {
        $('body').append('<div class="magic-cursor-wrapper"><div id="ball-cursor"><div id="ball-cursor-loader"></div></div></div>');
    
        var $cursorWrapper = $('.magic-cursor-wrapper'),
            $ballCursor = $('#ball-cursor'),
            $ballLoader = $('#ball-cursor-loader');
    
        // Add cursor style classes dynamically
        $('.magic-cursor').each(function () {
            var $this = $(this);
            if ($this.hasClass('light')) $cursorWrapper.addClass('magic-cursor-light');
            if ($this.hasClass('base-color')) $cursorWrapper.addClass('magic-cursor-base-color');
            if ($this.hasClass('view-cursor')) $cursorWrapper.addClass('magic-view-cursor');
            if ($this.hasClass('drag-cursor')) $cursorWrapper.addClass('magic-drag-cursor');
            if ($this.hasClass('round-cursor')) $cursorWrapper.addClass('magic-round-cursor');
        });
        ...
        function updatePosition() {
            if (!active) {
                ...
                gsap.to($ballCursor, {  // ✅ FIXED HERE
                    x: pos.x,
                    y: pos.y,
                    duration: 0.2, // Shorter duration for smoother tracking
                    ease: "power2.out", // Eases motion for a more natural feel
                });
            }
        }
    
        if (typeof gsap !== "undefined") {
            gsap.set($ballCursor, { xPercent: -50, yPercent: -50, scale: 0, opacity: 1 });
        ...
        $('.magic-cursor').on('mouseenter', function () {
            gsap.to($ballCursor, { borderWidth: '2px', scale: 1, duration: 0.3 });
            gsap.to($ballLoader, { borderWidth: '2px', top: 2, left: 2, duration: 0.2 });
            $cursorWrapper.addClass('sliderhover');
            if ($(this).hasClass('magic-cursor-vertical')) {
                $cursorWrapper.addClass('vertical');
            }
        }).on('mouseleave', function () {
            gsap.to($ballCursor, { borderWidth: '2px', scale: 1, borderColor: 'transparent', opacity: 1, duration: 0.3 });
            gsap.to($ballLoader, { borderWidth: '2px', top: 0, left: 0, duration: 0.2 });
            $cursorWrapper.removeClass('sliderhover');
        });
    
        // Hide magic cursor on interactive elements
        $(document).on('mouseenter', '.swiper-button-next, .swiper-button-prev, .swiper-pagination, a:not(.force-magic-cursor)', function () {
            gsap.to($cursorWrapper, { opacity: 0, duration: 0.2 });
        }).on('mouseleave', '.swiper-button-next, .swiper-button-prev, .swiper-pagination, a:not(.force-magic-cursor)', function () {
            gsap.to($cursorWrapper, { opacity: 1, duration: 0.2 });
        });
    }
...
document.addEventListener("DOMContentLoaded", function () {
    // Initialize the map
    var map = L.map('map').setView([25.1744, 55.3711], 13); // Latitude, Longitude, Zoom Level
    ...
    // Add a marker
    var marker = L.marker([25.1744, 55.3711]).addTo(map);

    // Add a popup to the marker
    marker.bindPopup("<b>Meydan Road, Nad Al Sheba</b><br>Dubai, United Arab Emirates").openPopup();
  });
